...
export async function registrar(email, password, nombre, rol) {
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: { nombre, rol },
      emailRedirectTo: 'https://valenlysm29.github.io/RESTAURANTE-SAZON/login.html'
    },
  });
  if (error) throw error;
  return data;
...
export function ocultarMenuPorRol(rol) {
  ...
  ['nav-dashboard','nav-platos','nav-pedidos','nav-cocina','nav-facturacion'].forEach(id => {
    ...
    // Todos los <li> del nav deben tener style="display:none" en el HTML por defecto.
    // Aquí solo MOSTRAMOS los que sí están permitidos; nunca dejamos nada visible de más.
